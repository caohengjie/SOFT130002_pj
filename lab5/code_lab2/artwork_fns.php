...
function get_timeuploaded($artworkID,$username)
{
    $conn = db_connect();
    $userID = get_userID($username);
    $query = "select timeuploaded from cart where artworkID='".$artworkID."' and userID = '".$userID."'";
    $result = @$conn->query($query);
    if(!$result)
    {
        return false;
    ...
    $row = $result->fetch_object();
    return $row->timeuploaded;
}

function get_view($artworkID)
{
    $conn = db_connect();
...
{
    $conn = db_connect();
    $query = "select * from artworks where artworkID<50
    order by view desc";
    $result = @$conn->query($query);
    if (!$result) {
        return false;
...
{
    $conn = db_connect();
    $query = "select * from artworks where artist='".$search_info."' or title='".$search_info."' or genre='".$search_info."' order by view desc" ;
    $result = @$conn->query($query);
    if (!$result) {
        return false;
    }
...
function get_arts_search_bytime($search_info)
{
    $conn = db_connect();
    $query = "select * from artworks where artist='".$search_info."' or title='".$search_info."' or genre='".$search_info."' order by timeReleased desc" ;
    $result = @$conn->query($query);
    if (!$result) {
        return false;
    }
...
function add_to_cart($artworkID,$username)
{
    $conn = db_connect();
    $userID = get_userID($username);
    $query = "insert into cart(userID,artworkID) values (".$userID.",".$artworkID.")";
    $result = $conn->query($query);
    if (!$result) {
